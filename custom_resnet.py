...
class BottleneckWide(nn.Module):
    expansion = 4

    def __init__(
            self, inplanes, planes, kern, stride=1, downsample=None, cardinality=1, base_width=64,
            reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
            attn_layer=None, aa_layer=None, drop_block=None, drop_path=None):
        super(BottleneckWide, self).__init__()
        print("BottleneckWide using kernel size ", kern)
        ...
        self.bn1 = norm_layer(first_planes)
        self.act1 = act_layer(inplace=True)
        #this is the main modification we're making 

        self.conv2 = nn.Conv2d(
            first_planes, width, kernel_size=kern, stride=1 if use_aa else stride,
            padding=padding, dilation=first_dilation, groups=cardinality, bias=False)
...
def make_blocks(
        block_fn, kern, channels, block_repeats, inplanes, reduce_first=1, output_stride=32,
    ...
    for stage_idx, (planes, num_blocks, db) in enumerate(zip(channels, block_repeats, drop_blocks(drop_block_rate))):
        ...
        for block_idx in range(num_blocks):
            ...
            blocks.append(block_fn(
                inplanes, planes,kern, stride, downsample, first_dilation=prev_dilation,
...
class ResNet(nn.Module):
    ...
    def __init__(
            self, block, layers, kern = 3, num_classes=1000, in_chans=3, output_stride=32, global_pool='avg',
        ...
        stage_modules, stage_feature_info = make_blocks(
            block, self.kern, channels, layers, inplanes, cardinality=cardinality, base_width=base_width,
...
@register_model
def kernresnet50(pretrained=False, kern = 3, **kwargs):
    """Constructs a ResNet-50 model.
    """
    model_args = dict(block=BottleneckWide, layers=[3, 4, 6, 3], kern = kern,  **kwargs)
    return _create_resnet('kernresnet50', pretrained, **model_args)
